...
function getCommitInfo(commitHash: string): { author: string; date: string; message: string } {
  try {
...
export async function scanGitHistory(
  dirPath: string,
  options: { verify?: boolean; maxCommits?: number } = {}
): Promise<SecretInCommit[]> {
  const results: SecretInCommit[] = [];
  const maxCommits = options.maxCommits || 1000;

  try {
    const logOutput = execSync(
      `git log --format="%H" -n ${maxCommits}`,
...
export function formatGitResults(results: SecretInCommit[]): string {
  if (results.length === 0) return 'No secrets found in git history.';
