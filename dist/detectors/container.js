...
function extractDockerConfig(image) {
    try {
...
async function extractImageLayers(image, destDir) {
    try {
...
async function scanFileContent(filePath, verify) {
    const results = [];
    try {
...
export async function scanContainerImage(image, options = { verify: false }) {
    const results = [];
    const configFiles = [];
        ...
        for (const layerDir of layers) {
            try {
                ...
                for (const file of files.trim().split('\n').filter(Boolean)) {
                    if (!fileTypesToScan.some(ext => file.endsWith(ext)))
                        continue;
...
export function formatContainerResults(result) {
    const lines = [];
