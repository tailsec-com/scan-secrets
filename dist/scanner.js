...
function isScannable(path) {
    if (!SCANNABLE_EXTENSIONS.some(ext => path.endsWith(ext)))
        return false;
    return !shouldExclude(path);
}
async function scanFile(content, filePath, verify) {
    const results = [];
...
export function formatResults(summary, format = 'text') {
    switch (format) {
        case 'json':
...
function formatText(summary) {
    ...
    for (const result of summary.results) {
        lines.push(`\n[${result.type}]`);
        if (result.file)
            lines.push(`  File: ${result.file}${result.line ? `:${result.line}` : ''}`);
        if (result.verified)
            lines.push(`  ✓ Verified`);
